...
/*void del_after(Node **start,int pos){
Node *current=search(*start,val);
if(current !=NULL){

    if(current->next==NULL){
        printf("Node not available after %d,deletion not possible",val);
    }
    else{
        Node *temp=current->next;
        current->next=temp->next;
        printf("\n Node deleted after %d with %d  value",pos,temp->val);
        free(temp);
    }
}
}*/
